  ...
  useEffect(() => {
    ...
    const getBudgets = async () => {
      try {
        ...
        setPercentOfIncomeBeingSaved(
          Math.round(budget?.yearlyGrossIncome / (savingAmount * 12))
        );
      } catch (err) {
        console.error(err);
      ...
      <div className="dashboard-info-panel">
        ...
        <p>
          ...
          {percentOfIncomeBeingSaved ? (
            <span>
              {" "}
              This is %{percentOfIncomeBeingSaved} of your yearly income.{" "}
            </span>
          ) : (
            <span> * </span>
          )}
          Investing this much with an average yearly return of %7 will result in
          a total of
          {compoundSavings ? (
